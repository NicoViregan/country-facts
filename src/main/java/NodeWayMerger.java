...
public class NodeWayMerger {
    ...
    private static Dataset<Row> explodeNodes(final Dataset<Row> wayDs) {
        return wayDs.select(explode(col("nodes")).as("each_node"), col("id"), col("tags"));
    }

    private static Dataset<Row> addIndexAndIdColumns(final Dataset<Row> explodedWay) {
        final Dataset<Row> result = explodedWay.withColumn("each_node_id", col("each_node.nodeId"))
                .withColumn("segment_in_way", col("each_node.index"))
                .select(col("each_node_id"), col("segment_in_way"), col("id"), col("tags"));
        return result;
    }

    private static Dataset<Row> join(final Dataset<Row> newWayDs, final Dataset<Row> nodeDs) {
        return newWayDs.join(nodeDs, newWayDs.col("each_node_id").equalTo(nodeDs.col("node_id")), "inner");
    }
